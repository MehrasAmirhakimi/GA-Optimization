...
Fun = [x(:, 1) + x(:, 2) - x(:, 6) ...;
    ...
    F - x(:, 10) - x(:, 5) - x(:, 2) ...;
    F*xF - x(:, 10).*x(:, 11) ...;
    x(:, 6)*HV0 + F*x(:, 15) - (x(:, 2)+x(:, 5)).*x(:, 3) - x(:, 7)*HD1 - x(:, 10).*x(:, 12) ...;
    P - x(:, 5) - x(:, 7) ...;
    x(:, 5).*x(:, 8) + x(:, 7)*HD1 - P*x(:, 9) ...;
    x(:, 5).*x(:, 3) + F*Hsw - x(:, 5).*x(:, 8) - F*x(:, 15) ...;
    x(:, 4) - x(:, 13) + BPE + dTfric ...;
    x(:, 3) - 2501.689845 - 1.806916015*x(:, 4) - 5.087717e-4*x(:, 4).^2 + 1.1221e-5*x(:, 4).^3 ...;
    x(:, 12) + 0.033635409 - 4.207557011*x(:, 13) + 6.200339e-4*x(:, 13).^2 - 4.459374e-6*x(:, 13).^3 + 1e-3*x(:, 11).*(-2.348e4 + 3.152e5*x(:, 11) + 2.308e6*x(:, 11).^2 - 1.446e7*x(:, 11).^3 + 7.826e3*x(:, 13) - 4.417e1*x(:, 13).^2 + 2.139e-1*x(:, 13).^3 - 1.991e4*x(:, 11).*x(:, 13) + 2.778e4*x(:, 11).^2.*x(:, 13) + 9.728e1*x(:, 11).*x(:, 13).^2) ...;
    dT * (log(TV0 - x(:, 4)) - log(TD1 - x(:, 14))) - (TV0 - x(:, 4) - TD1 + x(:, 14)) ...;
    x(:, 15) + 0.033635409 - 4.207557011*x(:, 14) + 6.200339e-4*x(:, 14).^2 - 4.459374e-6*x(:, 14).^3 + 1e-3*xF*(-2.348e4 + 3.152e5*xF + 2.308e6*xF^2 - 1.446e7*xF^3 + 7.826e3*x(:, 14) - 4.417e1 * x(:, 14).^2 + 2.139e-1*x(:, 14).^3 - 1.991e4*xF*x(:, 14) + 2.778e4*xF^2*x(:, 14) + 9.728e1*xF*x(:, 14).^2)];
Fun = Fun ./ repmat(x0, [size(x, 1), 1]);
%Fun = sum(Fun.^2, 2);
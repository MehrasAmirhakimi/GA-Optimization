...
npop = 250;
ngen = 250;
m = zeros(1, 15); % minimization problem: 0 or maximization problem: 1
id = repmat([0; 1.5], [1, 15]);
...
costfunction = @(x) myfun3(x); % this function has 2 outputs

%% Create initial population

initpop = zeros(npop, ncr + numel(m) + 2);
initpop(:, 1 : ncr) = createrandomsolution (npop, id);
initpop(:, ncr + 1 : ncr + numel(m)) = costfunction(initpop(:, 1 : ncr));
...
for i = 1:ngen
    ...
    crosspop(jx+1:end, :)=[];
    crosspop(:, ncr + 1 : ncr + numel(m)) = costfunction(crosspop(:, 1:ncr));
    ...
    mutpop(jm+1:end, :)=[];
    mutpop(:, ncr + 1 : ncr + numel(m)) = costfunction(mutpop(:, 1:ncr));

...
npop = 200;
ngen = 100;
m = [0, 0, 0]; % minimization problem: 0 or maximization problem: 1
id = repmat([-3; 3], [1, 2]);
...
for i = 1:ngen
    ...
    sffnum = ffnum(so);
    elitesnum = sffnum(1 : min(numel(sffnum), floor(npop / 2)));
    ne = numel(elitesnum);
